...
const MainLayout: React.FC<MainLayoutProps> = ({ children, className }) => {
  ...
  return (
    <div className="bg-slate-900 text-neutral-50">
      <header
        className={`w-full border-b border-slate-800 px-5 md:px-10 flex z-50 flex-row items-center justify-between fixed transform duration-300 bg-slate-900 ${
          scrollPosition > 185 ? 'h-12 bg-opacity-80' : 'h-16'
        }`}
      >
        <Link
          to="/"
        ...
        >
          <img
            ...
            className={`transform duration-300 ${
              scrollPosition > 185 ? 'h-8 w-8' : 'h-12 w-12'
            }`}
          />
        </Link>
        <nav
          className={`flex flex-col items-center bg-slate-900 md:bg-transparent w-full h-screen px-10 py-12 space-y-5 fixed top-[64px] z-50 left-0 transform duration-500 ${
        ...
        >
          ...
          >
            <span
              className={`font-black text-amber-500 transform duration-300 ${
                scrollPosition > 185 ? 'text-xs' : 'text-sm'
              }`}
            >{`01.`}</span>
            <p
              className={`font-semibold text-neutral-300 hover:text-amber-500 transition-all duration-300 transform ${
                scrollPosition > 185 ? 'text-xs' : 'text-sm'
              }`}
            >
              About
          ...
          >
            ...
            >{`02.`}</span>
            <p
              className={`text-sm font-semibold text-neutral-300 hover:text-amber-500 transition-all duration-300 transform ${
                scrollPosition > 185 ? 'text-xs' : 'text-sm'
              }`}
            >
              Portfolio
          ...
          >
            <a
              href="/"
              className={`border-2 border-amber-500 rounded-md font-bold text-neutral-50 bg-slate-900 hover:border-amber-700 transition-all duration-300 transform ${
                scrollPosition > 185
        ...
        >
          {isOpen ? (
            <RiCloseLine
              className={`text-amber-500 font-black transform duration-300 ${
                scrollPosition > 185 ? 'w-6 h-6' : 'w-8 h-8'
              }`}
            />
          ) : (
            <RiMenu4Line
      ...
      <main
        className={`min-h-fit w-full pt-20 px-5 md:px-10 lg:px-28 z-0 ${className} ${
      ...
      <footer
        className={`min-h-fit w-full z-0 flex flex-col justify-center items-center py-3 ${className} ${
          isOpen ? 'blur-sm' : 'blur-none'
        }`}
      >
      ...
      <button
        ...
        className={`fixed bottom-5 right-5 md:right-14 z-50 p-1 bg-slate-800 hover:bg-amber-500 transition-all ease-in-out duration-300 rounded-full flex flex-col justify-center items-center ${
          scrollPosition > 150 ? 'block' : 'hidden'
        }`}
      >
