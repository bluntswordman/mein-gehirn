	...
	return (
		<div className='bg-slate-900 text-neutral-50'>
			...
			>
				...
				<nav
					className={`flex flex-col items-center bg-slate-900 w-full h-screen px-10 py-12 space-y-5 fixed top-[64px] z-50 left-0 transform duration-500 ${
					...
					<div className='flex flex-row items-center justify-start w-full'>
						<a
							...
							className={`border-2 border-amber-500 rounded-md font-bold text-neutral-50 bg-slate-900 hover:border-amber-700 transition-all duration-300 transform ${
								scrollPosition > 185 ? 'px-2.5 py-0.5 text-xs' : 'px-3.5 py-1 text-sm'
							}`}
						>
							Resume
						</a>
				...
				>
					{isOpen ? (
						<RiCloseLine className={`text-amber-500 font-black transform duration-300 ${scrollPosition > 185 ? 'w-6 h-6' : 'w-8 h-8'}`} />
					) : (
						<RiMenu4Line className={`text-amber-500 font-black transform duration-300 ${scrollPosition > 185 ? 'w-6 h-6' : 'w-8 h-8'}`} />
					)}
				</button>
			</header>
